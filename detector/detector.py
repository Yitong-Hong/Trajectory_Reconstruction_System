...
class Detector:
    ...
    def load(self, cam_para_file, w, h, det_file):
        self.mapper = Mapper(cam_para_file, w, h)  # cam_para_file:
        self.load_detfile(det_file)
        # self.load_detfile_for_UAVDT(det_file)
    ...
    def load_detfile_for_UAVDT(self, filename):
        ...
        with open(filename, 'r') as f:
            ...
            for line in f.readlines():
                ...
                # print("type dets.key",self.dets.keys())
                # 将det添加到字典中
                if frame_id not in self.dets:
                    self.dets[frame_id] = []
                self.dets[frame_id].append(det)
    ...
    def get_dets(self, frame_id, conf_thresh=0, det_class=0):
        ...
        # print("dets.keys",self.dets.keys())
        if (frame_id not in self.dets):
            return []
