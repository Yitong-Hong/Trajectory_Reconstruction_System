...
def readCamParaFile(camera_para,w,h):
    R = np.zeros((3, 3))
    T = np.zeros((3, 1))
    ...
    try:
        ...
        while i < len(lines):
            if lines[i].strip() == "RotationMatrices":
                ...
                i += 1
        IntrinsicMatrix[0][2]=w/2
        IntrinsicMatrix[1][2]=h/2
...
class Mapper(object):
    def __init__(self, campara_file, w,h):
        self.A = np.zeros((3, 3))

        self.Ki, self.Ko, self.is_ok = readCamParaFile(campara_file,w,h)
