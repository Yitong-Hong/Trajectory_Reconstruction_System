from detector.detector_by_frame import Detector, Detection
...
def run_ucmc(args, det_path="det_results/mot17/yolox_x_ablation",
    ...
    eval_path = os.path.join(out_path, exp_name)  # output/mot17/val
    orig_save_path = os.path.join(eval_path, seq_name)  # output/mot17/val/MOT17-02
    if not os.path.exists(orig_save_path):
        os.makedirs(orig_save_path)

    if dataset == "MOT17":
        det_file = os.path.join(det_path, f"{seq_name}-SDP.txt")
        cam_para_path = os.path.join(cam_path, f"{seq_name}-SDP")  # cam_para/MOT17/MOT17-13-SDP 文件夹里是每frame的相机参数
        result_file = os.path.join(orig_save_path, f"{seq_name}-SDP.txt")  # output/mot17/val/MOT17-02/MOT17-13-SDP.txt
    elif dataset == "MOT20":
        ...
        det_file = os.path.join(det_path, f"{seq_name}.txt")
        cam_para = os.path.join(cam_path, f"{seq_name}")
        result_file = os.path.join(orig_save_path, f"{seq_name}.txt")
    gmc_file = None
    ...
    detector.load(cam_para_path, det_file, gmc_file)  # cam_para/MOT17/MOT17-13-SDP.txt
